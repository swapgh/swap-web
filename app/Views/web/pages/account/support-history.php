    ...
    <div class="auth-copy auth-account-hero auth-account-hero-wide">
      <div class="auth-hero-split">
        ...
        <div class="auth-kpi-strip">
          <article class="auth-kpi-card"><span class="auth-stat-label"><?= e($page['provider']) ?></span><strong><?= e(ucfirst($billingProviderName)) ?></strong></article>
          ...
          <article class="auth-kpi-card"><span class="auth-stat-label"><?= e($page['contributions']) ?></span><strong><?= e((string) count($sessions)) ?></strong></article>
          <article class="auth-kpi-card"><span class="auth-stat-label"><?= e($page['latest']) ?></span><strong><?= e($latestSession !== null ? $latestStatus['label'] : $page['no_records']) ?></strong></article>
        </div>
      </div>
    </div>
          ...
          <div class="auth-record-list auth-record-list-tight">
            <?php foreach ($recentSessions as $session): ?>
              ...
              <article class="auth-record-row">
                <div class="auth-record-main"><strong><?= e((string) ($session['product_key'] ?? 'supporter_tier')) ?></strong><span><?= e((string) ($session['id'] ?? '')) ?></span></div>
                <div class="auth-record-meta"><span class="auth-stat-label"><?= e($page['status']) ?></span><span class="auth-status-badge <?= e($status['class']) ?>"><?= e($status['label']) ?></span></div>
                <div class="auth-record-meta"><span class="auth-stat-label"><?= e($page['amount']) ?></span><strong><?= e(format_money_from_cents((int) ($session['amount_cents'] ?? 0), (string) ($session['currency'] ?? 'EUR'))) ?></strong></div>
                <div class="auth-record-meta"><span class="auth-stat-label"><?= e($page['customer']) ?></span><span><?= e((string) ($session['customer_email'] ?? '')) ?></span></div>
                <div class="auth-record-meta"><span class="auth-stat-label"><?= e($page['created']) ?></span><span><?= e($createdAt) ?></span></div>
                <div class="auth-record-action"><?php if (!empty($session['checkout_url'])): ?><a class="auth-secondary auth-link-button" href="<?= e((string) $session['checkout_url']) ?>"><?= e($page['open']) ?></a><?php endif; ?></div>
              </article>
            <?php endforeach; ?>
          </div>
          <?php if (count($sessions) > count($recentSessions)): ?><p class="auth-meta-note"><?= e(sprintf($page['showing_latest'], count($recentSessions))) ?></p><?php endif; ?>
        <?php endif; ?>
      </div>
    </div>
  </section>
